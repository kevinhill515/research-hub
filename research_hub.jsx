...
const [calImportText,setCalImportText]=useState(""); const [calFilter,setCalFilter]=useState("All"); const [repText,setRepText]=useState(""); const [repData,setRepData]=useState({}); const [portTab,setPortTab]=useState("GL"); const [fxText,setFxText]=useState(""); const [fxRates,setFxRates]=useState({}); const [calLastUpdated,setCalLastUpdated]=useState(""); const [calLastUpdatedBy,setCalLastUpdatedBy]=useState("");
function applyFxImport(){   if(!fxText.trim())return;   var lines=fxText.trim().split("").join("").split("
").map(function(l){return l.replace("
","");}).filter(function(l){return l.trim();});   var rates={};   lines.forEach(function(line){     var delim=line.indexOf("	")>=0?"	":",";     var parts=line.split(delim).map(function(s){return s.trim();});     if(parts.length>=2){       var pair=parts[0].toUpperCase();       var rate=parseFloat(parts[1]);       if(!isNaN(rate)){         var ccy=pair.replace("USD","");         rates[ccy]=rate;       }     }   });   setFxRates(rates);   alert("FX rates loaded for "+Object.keys(rates).length+" currencies.");   setFxText(""); } function applyRepImport(){if(!repText.trim())return;var lines=repText.trim().split("").join("").split("\n").map(function(l){return l.replace("\r","");}).filter(function(l){return l.trim();});var data={};lines.forEach(function(line){var delim=line.indexOf("\t")>=0?"\t":",";var parts=line.split(delim).map(function(s){return s.trim().replace(/^"|"$/g,"");});if(parts.length>=3){var account=parts[0].toUpperCase().trim();var ticker=parts[1].toUpperCase().trim();var shares=parseFloat(parts[2].replace(/,/g,""));var port=REP_ACCOUNTS[account];if(port&&ticker&&!isNaN(shares)){if(!data[port])data[port]={};data[port][ticker]=shares;}}});setRepData(data);alert("Rep account data loaded.");setRepText("");}  const [pendingVal,setPendingVal]=useState(null); const [entryComments,setEntryComments]=useState({}); const [newCommentText,setNewCommentText]=useState({});
