...
{tab==="calendar"&&(<div><div style={{fontSize:14,fontWeight:500,color:T.text,marginBottom:12}}>Upcoming Earnings — Next 30 Days</div><EarningsCalendar companies={companies} T={T}/><div style={{marginTop:20,...CARD}}><div style={{fontSize:13,fontWeight:500,color:T.text,marginBottom:4}}>Bulk import earnings dates</div><div style={{fontSize:12,color:T.textSec,marginBottom:8}}>Paste two columns: Ticker and Date (YYYY-MM-DD). One per line.</div><textarea value={calImportText||""} onChange={function(e){setCalImportText(e.target.value);}} placeholder="AAPL  2026-05-01  TSM  2026-04-17" style={{...TA(100),fontFamily:"monospace",marginBottom:8}}/><button onClick={applyCalImport} style={{fontSize:12,padding:"6px 14px",fontWeight:500}}>Import</button></div></div></div></div></div>)}  {tab==="dashboard"&&(<div> <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap",borderBottom:"1px solid "+T.border,paddingBottom:10}}>          {[["overview","Overview"],["sectors","Sector Breakdown"],["countries","Country Breakdown"],["overlap","Portfolio Overlap"],["quality","Data Quality"]].map(function(item){return <button key={item[0]} style={TABST(dashSubTab===item[0])} onClick={function(){setDashSubTab(item[0]);}}>{item[1]}</button>;})}
        </div>
