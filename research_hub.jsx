...
export default function App(){
  ...
  const [importError,setImportError]=useState(""); const [dataHubTab,setDataHubTab]=useState("backup"); const [valImportText,setValImportText]=useState(""); const [estImportText,setEstImportText]=useState("");
...
const searchRef=useRef();
  ...
  useEffect(function(){if(ready)supaUpsert("companies",{id:"shared",data:JSON.stringify(companies)});},[companies,ready]);   useEffect(function(){if(ready&&lastPriceUpdate)supaUpsert("meta",{key:"lastPriceUpdate",value:lastPriceUpdate});},[lastPriceUpdate,ready]);   useEffect(function(){if(ready)supaUpsert("meta",{key:"entryComments",value:JSON.stringify(entryComments)});},[entryComments,ready]);
...
function applyPriceImport(){
    if(!priceImportText.trim())return;var lines=priceImportText.trim().split("").join("").split("\n").map(function(l){return l.replace("\r","");}).filter(function(l){return l.trim();});var ordMap={};var adrMap={};
  var priceData=[];lines.forEach(function(line){var delim=line.indexOf("\t")>=0?"\t":",";var parts=line.split(delim).map(function(s){return s.trim().replace(/^"|"$/g,"");});if(parts.length>=3){var name=parts[0];var ordTicker=parts[1].toUpperCase();var ordPrice=parseFloat(parts[2]);var adrTicker=parts.length>=5&&parts[3]?parts[3].toUpperCase():"";var adrPrice=parts.length>=5&&parts[4]?parseFloat(parts[4]):NaN;priceData.push({name:name,ordTicker:ordTicker,ordPrice:ordPrice,adrTicker:adrTicker,adrPrice:isNaN(adrPrice)?null:adrPrice});}});
    var count=0;
    setCompanies(function(prev){return prev.map(function(c){var cname=(c.name||"").toLowerCase().trim();var match=priceData.find(function(d){return d.name.toLowerCase().trim()===cname;});if(!match)return c;var updates={};if(!isNaN(match.ordPrice)){updates.valuation=Object.assign({},c.valuation||{},{price:match.ordPrice});count++;}var newTickers=[{ticker:match.ordTicker,price:match.ordPrice,currency:(c.valuation&&c.valuation.currency)||getCurrency(c.country),isOrdinary:true}];if(match.adrTicker&&match.adrPrice!==null)newTickers.push({ticker:match.adrTicker,price:match.adrPrice,currency:"USD",isOrdinary:false});updates.tickers=newTickers;return Object.assign({},c,updates);});});
    setPriceImportText("");setShowPriceImport(false);setLastPriceUpdate(todayStr()+" "+new Date().toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"}));setTimeout(function(){alert("Updated prices for "+count+" companies.");},100);
  }
  function findDupes(){
...
{tab==="portfolios"&&(<div>   <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap",borderBottom:"1px solid "+T.border,paddingBottom:10}}>     {PORTFOLIOS.map(function(p){return <button key={p} style={TABST(portTab===p)} onClick={function(){setPortTab(p);}}>{PORT_NAMES[p]||p}</button>;})}   </div>   {(function(){     var portCos=companies.filter(function(c){return(c.portfolios||[]).indexOf(portTab)>=0;});     var portRep=repData[portTab]||{};     var totalMV=0;     portCos.forEach(function(c){       var allTickers=c.tickers||[];       allTickers.forEach(function(t){         var tk=(t.ticker||"").toUpperCase();         var shares=portRep[tk];         if(shares&&t.price){var ccy=(t.currency||"USD").toUpperCase();var fx=ccy==="USD"?1:(fxRates[ccy]||0);if(fx>0)totalMV+=shares*parseFloat(t.price)*fx;}       });     });     return(<div>       <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>         <span style={{fontSize:13,fontWeight:500,color:T.text}}>{PORT_NAMES[portTab]} — {portCos.length} companies</span>         <span style={{fontSize:12,color:T.textSec}}>Rep AUM: ${totalMV.toLocaleString(undefined,{maximumFractionDigits:0})} | Tickers: {Object.keys(portRep).join(", ")}</span>       </div>       <div style={{overflowX:"auto"}}><div style={{display:"table",width:"100%",borderCollapse:"separate",borderSpacing:"0 2px"}}>         <div style={{display:"table-row"}}>           {["Company","Next Report","MOS","Target %","Rep %","Diff"].map(function(h){return <div key={h} style={{display:"table-cell",fontSize:10,color:T.textSec,textTransform:"uppercase",letterSpacing:"0.05em",paddingBottom:6,paddingRight:12}}>{h}</div>;})}         </div>         {portCos.map(function(c){           var val=c.valuation||{};var ne=calcNormEPS(val)||parseFloat(val.eps);var tp=calcTP(val.pe,ne);var ordTicker=(c.tickers||[]).find(function(t){return t.isOrdinary;});var ordPrice=ordTicker?parseFloat(ordTicker.price):parseFloat(val.price);var mos=calcMOS(tp,ordPrice);var mosStyle=mosBg(mos);           var target=parseFloat((c.portWeights||{})[portTab])||0;           var allTickers=c.tickers||[];           var repMV=0;           allTickers.forEach(function(t){             var tk=(t.ticker||"").toUpperCase();             var shares=portRep[tk];             if(shares&&t.price){var ccy=(t.currency||"USD").toUpperCase();var fx=ccy==="USD"?1:(fxRates[ccy]||1);repMV+=shares*parseFloat(t.price)*fx;}           });           var repWeight=totalMV>0&&repMV>0?Math.round(repMV/totalMV*10000)/100:null;           var diff=repWeight!==null&&target>0?Math.round((repWeight-target)*100)/100:null;           var nextReport=null;           var today=new Date();today.setHours(0,0,0,0);           (c.earningsEntries||[]).forEach(function(e){if(!e.reportDate)return;var d=new Date(e.reportDate);if(d>=today){if(!nextReport||d<nextReport)nextReport=d;}});           var rowBg=diff!==null?(diff<=-0.3?"rgba(220,38,38,0.15)":diff>=0.5?"rgba(22,101,52,0.15)":T.bgSec):T.bgSec;           var td={display:"table-cell",verticalAlign:"middle",paddingRight:12,paddingTop:6,paddingBottom:6,fontSize:13,color:T.text,background:rowBg};           return(<div key={c.id} onClick={function(){setSelCo(c);setTab("companies");setCoView("section:Valuation");}} style={{display:"table-row",cursor:"pointer"}}>             <div style={{...td,fontWeight:500,cursor:"pointer"}}>{c.name}</div>             <div style={{...td,fontSize:12,color:nextReport?T.text:T.textSec}}>{nextReport?nextReport.toISOString().slice(0,10):"--"}</div>             <div style={td}>{mosStyle?<span style={{fontSize:11,padding:"2px 7px",borderRadius:99,background:mosStyle.bg,color:mosStyle.color,fontWeight:600}}>{fmtMOS(mos)}</span>:"--"}</div>             <div style={td}>{target>0?target+"%":"--"}</div>             <div style={td}>{repWeight!==null?repWeight+"%":"--"}</div>             <div style={{...td,fontWeight:600,color:diff===null?"":diff<=-0.3?(T.dark?"#f87171":"#dc2626"):diff>=0.5?(T.dark?"#4ade80":"#166534"):T.textSec}}>{diff!==null?(diff>0?"+":"")+diff+"%":"--"}</div>           </div>);         })} {(function(){   var cashShares=Object.entries(portRep).filter(function(e){return e[0]==="CASH";}).reduce(function(s,e){return s+e[1];},0);   var divShares=Object.entries(portRep).filter(function(e){return e[0]==="DIVACC";}).reduce(function(s,e){return s+e[1];},0);   var specialRows=[];   if(cashShares>0)specialRows.push({label:"CASH",mv:cashShares});   if(divShares>0)specialRows.push({label:"DIVACC",mv:divShares});   return specialRows.map(function(r){     var repWeight=totalMV>0?Math.round(r.mv/totalMV*10000)/100:null;     var td={display:"table-cell",verticalAlign:"middle",paddingRight:12,paddingTop:6,paddingBottom:6,fontSize:13,color:T.text,background:T.bgSec};     return(<div key={r.label} style={{display:"table-row"}}>       <div style={{...td,fontWeight:500}}>{r.label}</div>       <div style={td}>--</div>       <div style={td}>--</div>       <div style={td}>--</div>       <div style={td}>{repWeight!==null?repWeight+"%":"--"}</div>       <div style={td}>--</div>     </div>);   }); })()} </div></div>     </div>);   })()}   <div style={{marginTop:20,background:T.bgSec,borderRadius:8,border:"1px solid "+T.border,padding:"12px 14px",marginBottom:12}}><div style={{fontSize:13,fontWeight:500,color:T.text,marginBottom:4}}>FX Rates</div><div style={{fontSize:12,color:T.textSec,marginBottom:8}}>Paste two columns: Pair (e.g. GBPUSD) and Rate. {Object.keys(fxRates).length>0&&Object.keys(fxRates).length+" currencies loaded."}</div><textarea value={fxText} onChange={function(e){setFxText(e.target.value);}} placeholder="GBPUSD  1.3463" rows={4} style={{width:"100%",resize:"vertical",fontSize:13,padding:"8px 10px",boxSizing:"border-box",borderRadius:6,border:"1px solid "+T.border,background:T.bg,color:T.text,fontFamily:"inherit",lineHeight:1.6,marginBottom:8}}></textarea><button onClick={applyFxImport} style={{fontSize:12,padding:"6px 14px",fontWeight:500}}>Load FX</button></div> <div style={{background:T.bgSec,borderRadius:8,border:"1px solid "+T.border,padding:"12px 14px"}}>     <div style={{fontSize:13,fontWeight:500,color:T.text,marginBottom:4}}>Upload rep account holdings</div>     <div style={{fontSize:12,color:T.textSec,marginBottom:8}}>Paste three columns: Account Number, Ticker, Shares. Data is not saved between sessions.</div>     <textarea value={repText} onChange={function(e){setRepText(e.target.value);}} placeholder="LWGA0013  SHEL  1500" rows={5} style={{width:"100%",resize:"vertical",fontSize:13,padding:"8px 10px",boxSizing:"border-box",borderRadius:6,border:"1px solid "+T.border,background:T.bg,color:T.text,fontFamily:"inherit",lineHeight:1.6,marginBottom:8}}></textarea><button onClick={applyRepImport} style={{fontSize:12,padding:"6px 14px",fontWeight:500}}>Load</button></div>)}
        ...
        return(<div>
          ...
          {coView.startsWith("section:")&&(function(){
            ...
            return(<div>
              ...
              {isOverview&&(<div style={{marginBottom:16}}><div style={{fontSize:12,fontWeight:500,color:T.textSec,marginBottom:8,textTransform:"uppercase",letterSpacing:"0.05em"}}>Tickers & Prices</div><div style={{fontSize:11,color:T.textSec,marginBottom:8}}>Add all tickers for this security. Mark the ordinary share used for TP/MOS.</div>{(function(){var co=companies.find(function(c){return c.id===selCo.id;})||selCo;return co.tickers||(co.ticker?[{ticker:co.ticker,price:(co.valuation&&co.valuation.price)||"",currency:(co.valuation&&co.valuation.currency)||getCurrency(co.country),isOrdinary:true}]:[{ticker:"",price:"",currency:"",isOrdinary:true}]);})().map(function(t,i){var tickers=selCo.tickers||[{ticker:"",price:"",currency:"",isOrdinary:true}];function updTicker(patch){var nt=tickers.slice();nt[i]=Object.assign({},nt[i],patch);var u=Object.assign({},selCo,{tickers:nt});setSelCo(u);setCompanies(function(cs){return cs.map(function(c){return c.id===u.id?u:c;});});}return(<div key={i} style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}><input value={t.ticker||""} onChange={function(e){updTicker({ticker:e.target.value.toUpperCase()});}} placeholder="Ticker" style={{fontSize:12,padding:"4px 8px",borderRadius:5,border:"1px solid "+T.border,background:T.bg,color:T.text,width:90}}/><input value={t.price?fmtPrice(t.price):""} onChange={function(e){updTicker({price:e.target.value.replace(/,/g,"")});}} placeholder="Price" style={{fontSize:12,padding:"4px 8px",borderRadius:5,border:"1px solid "+T.border,background:T.bg,color:T.text,width:90}}/><select value={t.currency||""} onChange={function(e){updTicker({currency:e.target.value});}} style={{fontSize:12,padding:"4px 8px",borderRadius:5,border:"1px solid "+T.border,background:T.bg,color:T.text}}><option value="">CCY</option>{ALL_CURRENCIES.map(function(c){return <option key={c}>{c}</option>;})}</select><label style={{fontSize:11,color:T.textSec,display:"flex",alignItems:"center",gap:4,cursor:"pointer"}}><input type="radio" checked={!!t.isOrdinary} onChange={function(){var nt=tickers.map(function(x,j){return Object.assign({},x,{isOrdinary:j===i});});var u=Object.assign({},selCo,{tickers:nt});setSelCo(u);setCompanies(function(cs){return cs.map(function(c){return c.id===u.id?u:c;});});}}/>Ordinary</label>{tickers.length>1&&<span onClick={function(){var nt=tickers.filter(function(_,j){return j!==i;});var u=Object.assign({},selCo,{tickers:nt});setSelCo(u);setCompanies(function(cs){return cs.map(function(c){return c.id===u.id?u:c;});});}} style={{fontSize:11,color:T.textDanger,cursor:"pointer"}}>×</span>}</div>);})} <button onClick={function(){var nt=(selCo.tickers||[]).concat([{ticker:"",price:"",currency:"",isOrdinary:false}]);var u=Object.assign({},selCo,{tickers:nt});setSelCo(u);setCompanies(function(cs){return cs.map(function(c){return c.id===u.id?u:c;});});}} style={{fontSize:11,padding:"3px 10px",marginTop:4}}>+ Add ticker</button></div>)} <SectionEditTab title={sectionName} content={selCo.sections&&selCo.sections[sectionName]} onSave={function(newContent){var ns=Object.assign({},selCo.sections,{[sectionName]:newContent});var u=Object.assign({},selCo,{sections:ns,lastUpdated:todayStr()});setSelCo(u);setCompanies(function(cs){return cs.map(function(c){return c.id===u.id?u:c;});});}} T={T}/>
            </div>);
          }())}
