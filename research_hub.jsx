...
{tab==="calendar"&&(<div><div style={{fontSize:14,fontWeight:500,color:T.text,marginBottom:12}}>Upcoming Earnings — Next 30 Days</div><EarningsCalendar companies={companies} T={T}></EarningsCalendar><div style={{marginTop:20,background:T.bgSec,borderRadius:8,border:"1px solid "+T.border,padding:"12px 14px",marginBottom:8}}><div style={{fontSize:13,fontWeight:500,color:T.text,marginBottom:4}}>Bulk import earnings dates</div><div style={{fontSize:12,color:T.textSec,marginBottom:8}}>Paste two columns: Ticker and Date. One per line.</div><textarea value={calImportText||""} onChange={function(e){setCalImportText(e.target.value);}} placeholder="AAPL  2026-05-01" rows={5} style={{width:"100%",resize:"vertical",fontSize:13,padding:"8px 10px",boxSizing:"border-box",borderRadius:6,border:"1px solid "+T.border,background:T.bg,color:T.text,fontFamily:"inherit",lineHeight:1.6,marginBottom:8}}></textarea><button onClick={applyCalImport} style={{fontSize:12,padding:"6px 14px",fontWeight:500}}>Import</button></div></div>)}                    {tab==="dashboard"&&(<div><div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap",borderBottom:"1px solid "+T.border,paddingBottom:10}}>          {[["overview","Overview"],["sectors","Sector Breakdown"],["countries","Country Breakdown"],["overlap","Portfolio Overlap"],["quality","Data Quality"]].map(function(item){return <button key={item[0]} style={TABST(dashSubTab===item[0])} onClick={function(){setDashSubTab(item[0]);}}>{item[1]}</button>;})}
        </div>
