...
const ACTIONS=["Increase TP","No Action","Decrease TP"]; const TEAM_MEMBERS=["Chris","Al","Bob","Kevin","Ron","Emily"]; const FLAG_STYLES={"Needs Review":{bg:"#fef9c3",color:"#854d0e",icon:"⚑"},"Urgent":{bg:"#fee2e2",color:"#991b1b",icon:"🔴"}};
...
function applyPriceImport(){
    ...
    lines.forEach(function(line){var delim=line.indexOf("\t")>=0?"\t":",";var parts=line.split(delim).map(function(s){return s.trim().replace(/^"|"$/g,"");});if(parts.length>=2){var ordTicker=parts[0].toUpperCase();var ordPrice=parseFloat(parts[1]);if(ordTicker&&!isNaN(ordPrice))ordMap[ordTicker]=ordPrice;if(parts.length>=4&&parts[2]&&parts[3]){var adrTicker=parts[2].toUpperCase();var adrPrice=parseFloat(parts[3]);if(adrTicker&&!isNaN(adrPrice))adrMap[adrTicker]=adrPrice;}}});
    var count=0;
    setCompanies(function(prev){return prev.map(function(c){var ordTicker=(c.ticker||"").toUpperCase();var updates={};if(ordMap[ordTicker]!==undefined){updates.valuation=Object.assign({},c.valuation||{},{price:ordMap[ordTicker]});count++;}var existingTickers=(c.tickers||[]);var newTickers=existingTickers.map(function(t){var tk=(t.ticker||"").toUpperCase();if(ordMap[tk]!==undefined)return Object.assign({},t,{price:ordMap[tk]});if(adrMap[tk]!==undefined)return Object.assign({},t,{price:adrMap[tk]});return t;});if(ordMap[ordTicker]!==undefined){var hasOrd=newTickers.some(function(t){return(t.ticker||"").toUpperCase()===ordTicker;});if(!hasOrd)newTickers=[{ticker:ordTicker,price:ordMap[ordTicker],currency:(c.valuation&&c.valuation.currency)||getCurrency(c.country),isOrdinary:true}].concat(newTickers);}Object.keys(adrMap).forEach(function(adrTicker){var line=lines.find(function(l){var p=(l.indexOf("	")>=0?l.split("	"):l.split(",")).map(function(s){return s.trim();});return p[0].toUpperCase()===ordTicker&&p[2]&&p[2].toUpperCase()===adrTicker;});if(!line)return;var hasAdr=newTickers.some(function(t){return(t.ticker||"").toUpperCase()===adrTicker;});if(!hasAdr)newTickers=newTickers.concat([{ticker:adrTicker,price:adrMap[adrTicker],currency:"USD",isOrdinary:false}]);else newTickers=newTickers.map(function(t){return(t.ticker||"").toUpperCase()===adrTicker?Object.assign({},t,{price:adrMap[adrTicker]}):t;});});updates.tickers=newTickers;if(Object.keys(updates).length>0)return Object.assign({},c,updates);return c;});});
    if(selCo){setSelCo(function(prev){var ordTicker=(prev.ticker||"").toUpperCase();var updates={};if(ordMap[ordTicker]!==undefined){updates.valuation=Object.assign({},prev.valuation||{},{price:ordMap[ordTicker]});}var existingTickers=(prev.tickers||[]);var newTickers=existingTickers.slice();if(ordMap[ordTicker]!==undefined){var hasOrd=newTickers.some(function(t){return(t.ticker||"").toUpperCase()===ordTicker;});if(!hasOrd)newTickers=[{ticker:ordTicker,price:ordMap[ordTicker],currency:(prev.valuation&&prev.valuation.currency)||getCurrency(prev.country),isOrdinary:true}].concat(newTickers);}updates.tickers=newTickers;return Object.assign({},prev,updates);});}setPriceImportText("");setShowPriceImport(false);setLastPriceUpdate(todayStr()+" "+new Date().toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"}));setTimeout(function(){alert("Updated prices for "+count+" companies.");},100);
  }
  function findDupes(){
