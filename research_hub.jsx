...
const [calImportText,setCalImportText]=useState(""); const [calFilter,setCalFilter]=useState("All"); const [repText,setRepText]=useState(""); const [repData,setRepData]=useState({}); const [portTab,setPortTab]=useState("GL"); const [fxText,setFxText]=useState(""); const [fxRates,setFxRates]=useState({}); const [calLastUpdated,setCalLastUpdated]=useState(""); const [calLastUpdatedBy,setCalLastUpdatedBy]=useState(""); const [pendingVal,setPendingVal]=useState(null); const [entryComments,setEntryComments]=useState({}); const [newCommentText,setNewCommentText]=useState({});
function applyFxImport(){if(!fxText.trim())return;var lines=fxText.trim().split("").join("").split("\n").map(function(l){return l.replace("\r","");}).filter(function(l){return l.trim();});var rates={};lines.forEach(function(line){var delim=line.indexOf("\t")>=0?"\t":",";var parts=line.split(delim).map(function(s){return s.trim();});if(parts.length>=2){var pair=parts[0].toUpperCase();var rate=parseFloat(parts[1]);if(!isNaN(rate)){var ccy=pair.replace("USD","");rates[ccy]=rate;}}});setFxRates(rates);alert("FX rates loaded for "+Object.keys(rates).length+" currencies.");setFxText("");}
