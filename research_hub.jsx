...
function CoRow({company,onSelect,onDelete,onUpdate,compact,visibleCols,selected,onToggleSelect,onQuickUpload,T}){
  ...
  return(<div onClick={function(){onSelect(company);}} onMouseEnter={function(){setHovered(true);}} onMouseLeave={function(){setHovered(false);}} style={{display:"table-row"}}>
    ...
    {show("Portfolio")&&<div style={{...td,whiteSpace:"nowrap"}}><div style={{display:"flex",gap:3,alignItems:"center",flexWrap:"nowrap"}}><PortPicker active={portfolios} onChange={function(v){onUpdate(company.id,{portfolios:v});}} pillBg="#166534" pillColor="#fff" plusColor="#4ade80"/><PortPicker active={portNote} onChange={function(v){onUpdate(company.id,{portNote:v.join(", ")});}} plusColor="#1a3a6b" opts={availPortNote} dashedPills pillStyleFn={function(){return{bg:"transparent",color:"#1a3a6b"};}}/></div></div>}
