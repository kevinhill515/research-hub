...
const [calImportText,setCalImportText]=useState(""); const [calFilter,setCalFilter]=useState("All"); const [repText,setRepText]=useState(""); const [repData,setRepData]=useState({}); const [portTab,setPortTab]=useState("GL"); const [calLastUpdated,setCalLastUpdated]=useState(""); const [calLastUpdatedBy,setCalLastUpdatedBy]=useState("");
function applyRepImport(){   if(!repText.trim())return;   var lines=repText.trim().split("").join("").split("
").map(function(l){return l.replace("
","");}).filter(function(l){return l.trim();});   var data={};   lines.forEach(function(line){     var delim=line.indexOf("	")>=0?"	":",";     var parts=line.split(delim).map(function(s){return s.trim().replace(/^"|"$/g,"");});     if(parts.length>=3){       var account=parts[0].toUpperCase().trim();       var ticker=parts[1].toUpperCase().trim();       var shares=parseFloat(parts[2].replace(/,/g,""));       var port=REP_ACCOUNTS[account];       if(port&&ticker&&!isNaN(shares)){         if(!data[port])data[port]={};         data[port][ticker]=shares;       }     }   });   setRepData(data);   alert("Rep account data loaded.");   setRepText(""); } function applyCalImport(){if(!calImportText||!calImportText.trim())return;var calUpdatedBy=currentUser||"Unknown";var calUpdatedAt=todayStr()+" "+new Date().toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"});var lines=calImportText.trim().split("").join("").split("\n").map(function(l){return l.replace("\r","");}).filter(function(l){return l.trim();});var count=0;setCompanies(function(prev){return prev.map(function(c){var ticker=(c.ticker||"").toUpperCase();var match=lines.find(function(l){var parts=(l.indexOf("\t")>=0?l.split("\t"):l.split(",")).map(function(s){return s.trim();});return parts[0].toUpperCase()===ticker;});if(!match)return c;var parts=(match.indexOf("\t")>=0?match.split("\t"):match.split(",")).map(function(s){return s.trim();});var date=parts[1];if(!date)return c;var entries=(c.earningsEntries||[]).slice();var existing=entries.find(function(e){return e.reportDate===date;});if(existing)return c;var newEntry=Object.assign(blankEarnings(),{reportDate:date,quarter:"",open:false});entries.unshift(newEntry);entries.sort(function(a,b){var da=parseDate(a.reportDate),db=parseDate(b.reportDate);if(!da&&!db)return 0;if(!da)return 1;if(!db)return -1;return db.getTime()-da.getTime();});count++;return Object.assign({},c,{earningsEntries:entries});});});setCalLastUpdated(calUpdatedAt);setCalLastUpdatedBy(calUpdatedBy);supaUpsert("meta",{key:"calLastUpdated",value:calUpdatedBy+" at "+calUpdatedAt});setTimeout(function(){alert("Updated "+count+" companies.");setCalImportText("");},100);}
