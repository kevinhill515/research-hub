...
export default function App(){
  ...
  const [importError,setImportError]=useState(""); const [dataHubTab,setDataHubTab]=useState("backup"); const [valImportText,setValImportText]=useState("");
...
","");}).filter(function(l){return l.trim();});   var count=0;   setCompanies(function(prev){return prev.map(function(c){     var cname=(c.name||"").toLowerCase().trim();     var match=lines.find(function(l){var delim=l.indexOf("	")>=0?"	":",";var parts=l.split(delim).map(function(s){return s.trim().replace(/^"|"$/g,"");});return parts[0].toLowerCase().trim()===cname;});     if(!match)return c;     var delim=match.indexOf("	")>=0?"	":",";     var p=match.split(delim).map(function(s){return s.trim().replace(/^"|"$/g,"");});     var newVal=Object.assign({},c.valuation||{},{pe:p[1]||"",fyMonth:p[2]||"",currency:p[3]||"",fy1:p[4]||"",eps1:p[5]||"",w1:p[6]||"",fy2:p[7]||"",eps2:p[8]||"",w2:p[9]||""});     count++;     return Object.assign({},c,{valuation:newVal});   });});   setTimeout(function(){alert("Updated valuation for "+count+" companies.");setValImportText("");},100); } function applyFxImport(){if(!fxText.trim())return;var lines=fxText.trim().split("").join("").split("\n").map(function(l){return l.replace("\r","");}).filter(function(l){return l.trim();});var rates={};lines.forEach(function(line){var delim=line.indexOf("\t")>=0?"\t":",";var parts=line.split(delim).map(function(s){return s.trim();});if(parts.length>=2){var pair=parts[0].toUpperCase();var rate=parseFloat(parts[1]);if(!isNaN(rate)){var ccy=pair.replace("USD","");rates[ccy]=rate;}}});setFxRates(rates);supaUpsert("meta",{key:"fxRates",value:JSON.stringify(rates)});setFxLastUpdated(currentUser+" at "+todayStr()+" "+new Date().toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"}));alert("FX rates loaded for "+Object.keys(rates).length+" currencies.");setFxText("");}
function applyRepImport(){if(!repText.trim())return;var lines=repText.trim().split("").join("").split("\n").map(function(l){return l.replace("\r","");}).filter(function(l){return l.trim();});var data={};lines.forEach(function(line){var delim=line.indexOf("\t")>=0?"\t":",";var parts=line.split(delim).map(function(s){return s.trim().replace(/^"|"$/g,"");});if(parts.length>=3){var account=parts[0].toUpperCase().trim();var ticker=parts[1].toUpperCase().trim();var shares=parseFloat(parts[2].replace(/,/g,""));var port=REP_ACCOUNTS[account];if(port&&ticker&&!isNaN(shares)){if(!data[port])data[port]={};data[port][ticker]=shares;}}});setRepData(data);supaUpsert("meta",{key:"repData",value:JSON.stringify(data)});setRepLastUpdated(currentUser+" at "+todayStr()+" "+new Date().toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"}));alert("Rep account data loaded.");setRepText("");}
const searchRef=useRef();  function handleSortClick(colSort){
