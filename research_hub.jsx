...
export default function App(){
  const [authed,setAuthed]=useState(function(){try{return sessionStorage.getItem("rh_auth")==="1";}catch(e){return false;}}); if(!authed){return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#0f172a",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{background:"#1e293b",border:"1px solid #334155",borderRadius:12,padding:32,width:320}}><div style={{fontSize:16,fontWeight:600,color:"#f1f5f9",marginBottom:16}}>Research Hub</div><input type="password" id="pw" placeholder="Password" style={{width:"100%",boxSizing:"border-box",fontSize:14,padding:"8px 10px",borderRadius:6,border:"1px solid #334155",background:"#0f172a",color:"#f1f5f9",marginBottom:12}} onKeyDown={function(e){if(e.key==="Enter"){if(e.target.value==="LongWalkPartners2026"){try{sessionStorage.setItem("rh_auth","1");}catch(ex){}setAuthed(true);}else{alert("Wrong password");}}}} /><button onClick={function(){var v=document.getElementById("pw").value;if(v==="LongWalkPartners2026"){try{sessionStorage.setItem("rh_auth","1");}catch(ex){}setAuthed(true);}else{alert("Wrong password");}}} style={{width:"100%",padding:"9px",fontWeight:600,background:"#1e40af",color:"#fff",border:"none",borderRadius:6,cursor:"pointer"}}>Enter</button></div></div>);}(function(){try{return localStorage.getItem("rh_dark")==="1";}catch(e){return false;}}); const [currentUser,setCurrentUser]=useState(function(){try{return localStorage.getItem("rh_user")||"";}catch(e){return "";}}); const [showUserPicker,setShowUserPicker]=useState(false); useEffect(function(){try{localStorage.setItem("rh_dark",dark?"1":"0");}catch(e){};},[dark]); useEffect(function(){try{if(currentUser)localStorage.setItem("rh_user",currentUser);}catch(e){};},[currentUser]);
  ...
  useEffect(function(){if(ready)supabase.from("companies").upsert({id:"shared",data:JSON.stringify(companies)});},[companies,ready]);,JSON.stringify(companies)).catch(function(){});},[companies,ready]); useEffect(function(){if(ready&&lastPriceUpdate)supabase.from("meta").upsert({key:"lastPriceUpdate",value:lastPriceUpdate});},[lastPriceUpdate,ready]);,lastPriceUpdate).catch(function(){});},[lastPriceUpdate,ready]); useEffect(function(){if(ready)supabase.from("meta").upsert({key:"entryComments",value:JSON.stringify(entryComments)});},[entryComments,ready]);,JSON.stringify(entryComments)).catch(function(){});},[entryComments,ready]);
  ...
  function importAll(){
    setImportError("");
    try{var d=JSON.parse(importText);var cos=d.companies||(Array.isArray(d)?d:null),lib=d.library||null;if(!cos&&!lib){setImportError("No data found.");return;}if(cos&&Array.isArray(cos)){setCompanies(cos);supabase.from("companies").upsert({id:"shared",data:JSON.stringify(cos)}).catch(function(){});}if(lib&&Array.isArray(lib)){setSaved(lib);supabase.from("library").upsert({id:"shared",data:JSON.stringify(lib)}).catch(function(){});}setImportText("");setShowDataPanel(false);}
    ...
    <div style={{padding:"1rem",boxSizing:"border-box",fontFamily:"system-ui,sans-serif",fontSize:14,color:T.text,background:T.bg,minHeight:"100vh"}}>       {(!currentUser||showUserPicker)&&(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{background:T.bg,border:"1px solid "+T.border,borderRadius:12,padding:28,width:320,boxShadow:"0 8px 32px rgba(0,0,0,0.2)"}}><div style={{fontSize:16,fontWeight:600,color:T.text,marginBottom:6}}>Who are you?</div><div style={{fontSize:13,color:T.textSec,marginBottom:16}}>Select your name so edits are tracked correctly.</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{TEAM_MEMBERS.map(function(name){return(<button key={name} onClick={function(){setCurrentUser(name);setShowUserPicker(false);}} style={{padding:"10px 16px",fontSize:14,fontWeight:currentUser===name?600:400,background:currentUser===name?"#dbeafe":T.bgSec,color:currentUser===name?"#1e40af":T.text,border:"1px solid "+(currentUser===name?"#93c5fd":T.border),borderRadius:8,cursor:"pointer",textAlign:"left"}}>{name}</button>);})}</div>{currentUser&&<div style={{marginTop:12,fontSize:12,color:T.textSec,textAlign:"right",cursor:"pointer"}} onClick={function(){setShowUserPicker(false);}}>Cancel</div>}</div></div>)}
      ...
      {tab==="companies"&&!selCo&&(<div>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10,justifyContent:"flex-end",alignItems:"center"}}>
          {confirmClear?(<div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:12,color:T.textDanger}}>Delete all {companies.length}?</span><button onClick={async function(){setCompanies([]);try{await supabase.from("companies").upsert({id:"shared",data:"[]"});}catch(e){}setConfirmClear(false);}} style={{fontSize:12,padding:"4px 10px",color:T.textDanger}}>Yes</button><span onClick={function(){setConfirmClear(false);}} style={LNK}>Cancel</span></div>):<button onClick={function(){setConfirmClear(true);}} style={{fontSize:12,padding:"6px 10px",color:T.textDanger}}>Clear all</button>}
