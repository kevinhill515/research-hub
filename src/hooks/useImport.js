...
export function useImport(){
  ...
  function applyTxImport(){
    ...
    function normalize(n){return(n||"").toLowerCase().replace(/\b(corporation|incorporated|international|holdings|holding|company|limited|group)\b/g,"").replace(/\b(co\.|inc\.|ltd\.|llc|plc|sa|ag|nv|se|co|inc|ltd|corp|gmbh|kgaa|ab|asa|oyj|spa|srl|bv)\b/g,"").replace(/[.,&'()\-\/]/g," ").replace(/\s+/g," ").trim();}
    var byName={},byNorm={};
    ...
    setTimeout(function(){var msg="Imported "+txCount+" transactions across "+Object.keys(matchedNames).length+" companies.";if(unmatched.size>0)msg+="\n\nUnmatched names ("+unmatched.size+"):\n"+Array.from(unmatched).slice(0,10).join(", ")+(unmatched.size>10?" \u2026":"");alert(msg);setTxText("");},100);
  }
