  ...
  return (
    <div>
        ...
        <div className="flex gap-1.5 mb-2 items-center flex-wrap">
          ...
          {["All","Own","Focus","Watch","Sold","Removed"].map(function(s){var active=coStatusFilter===s;var cfg={All:{bg:undefined,color:undefined},Own:{bg:"#dcfce7",color:"#166534"},Focus:{bg:"#dbeafe",color:"#1e40af"},Watch:{bg:"#fef9c3",color:"#854d0e"},Sold:{bg:"#fee2e2",color:"#991b1b"},Removed:{bg:"#e2e8f0",color:"#475569"}}[s];return <span key={s} onClick={function(){setCoStatusFilter(s);setCoStatusSubFilter("All");}} className={"text-[11px] px-2.5 py-0.5 rounded-full cursor-pointer transition-colors " + (active?"font-semibold":"font-normal")} style={{border:"1px solid "+(active&&cfg.color?cfg.color:undefined),background:active?cfg.bg:undefined,color:active?cfg.color:undefined}}>{s}</span>;})}
          {(function(){
            if(coStatusFilter==="All")return null;
            var subOpts=coStatusFilter==="Own"?["FIN","IN","FGL","GL","EM","SC"]:coStatusFilter==="Focus"?["MC","EM","SC"]:coStatusFilter==="Watch"?["MC","EM","SC"]:coStatusFilter==="Sold"?["Hit TP","Gave Up"]:[];
            var cfg=({Own:{bg:"#dcfce7",color:"#166534"},Focus:{bg:"#dbeafe",color:"#1e40af"},Watch:{bg:"#fef9c3",color:"#854d0e"},Sold:{bg:"#fee2e2",color:"#991b1b"},Removed:{bg:"#e2e8f0",color:"#475569"}})[coStatusFilter];
